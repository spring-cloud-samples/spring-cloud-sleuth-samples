...
class RemoteFieldsTestChecker {
	...
	RemoteFieldsTestChecker(RestTemplate restTemplate) {
		...
		this.wireMockServer.stubFor(WireMock.get(WireMock.urlEqualTo("/foo")).withHeader("myremotefield", WireMock.equalTo("my-remote-field-value")).willReturn(WireMock.aResponse().withStatus(200).withBody("OK")));
	}

	void assertThatFieldGotPropagated() {
		String object = this.restTemplate.getForObject("http://localhost:" + this.wireMockServer.port() + "/foo", String.class);
		if (!"OK".equals(object)) {
