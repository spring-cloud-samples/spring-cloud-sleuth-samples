...
public class ReactiveContinuedTransactionService {
	...
	public ReactiveContinuedTransactionService(ReactiveCustomerRepository repository, Tracer tracer, ReactiveNestedTransactionService reactiveNestedTransactionService) {
	...
	public Mono<Void> continuedTransaction() {
		return Mono.fromRunnable(() -> log.info("<ACCEPTANCE_TEST> <TRACE:{}> Hello from consumer", tracer.currentSpan().context().traceId()))
				.then(repository.findById(1L))
				.doOnNext(customer -> {
